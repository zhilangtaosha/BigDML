...
USAGE:
  ...
  Models: Nbayes, RTrees, KNearest, Boost, SVM, MLP
'''
...
if __name__ == '__main__':
    ...
    models = [Nbayes,RTrees, KNearest, Boost, SVM, MLP] # NBayes
    ...
    if args['--model']!='nbayes':
        ...
        train_rate = np.mean(model.predict(samples[:train_n])[1].ravel() ==responses[:train_n])
        test_rate  = np.mean(model.predict(samples[train_n:])[1].ravel() ==responses[train_n:])
