...
if __name__ == '__main__':
    ...
    #u.item是电影的属性（电影名，上映日期，类别等）
    movies=getMoviesList("E://Code//SVN//Python//data//ml-100k//u.item")  #
    #u.data是用户对电影的评分行为（用户名，电影名，评分），其中179是用户的id,80是邻居的数量
    recommend_list,user_movie,items_movie,neighbors=recommendByUserFC("E://Code//SVN//Python//data//ml-100k//u.data",180,80)  #用户数据
    ...
    for movie_id in recommend_list[:20]:
        ...
        for user_id in items_movie[movie_id]:
            if user_id in neighbors_id:
                from_user.append(user_id)  #来自谁的推荐
